...
class NetflixAPIV1(_NetflixAPI):
    ...
    def get_catalog(self, stream=False):
        if stream:
            return get_catalog_stream(self)
        ...
        r = self._request("get", url_path, headers={'Accept-Encoding': 'gzip'}, data={'output': 'json'})
        return r
...
class NetflixAPIV2(_NetflixAPI):
    ...
    def get_catalog_stream(self, file=True, local_filename='my_file.out'):
        """Retrieve a complete index of all instant-watch titles in the Netflix catalog
           Stream to file or stdout for pipeline operations

        url: /catalog/titles/full

        :Returns:
            Returns catalog streamed to file or stdout for pipeline operations
        """
        url_path = '/catalog/titles/full'
        r = self._request("get", url_path, headers={'Accept-Encoding': 'gzip'}, data={'output': None})
